...
- The label `t.lumiSays` is a new bilingual i18n key (en: "?? Princess Lumi says:" / zh: "?? ?�米?�主話�?") added to `lib/i18n.ts` (both the `Dict` type and the `en`/`zh` dictionaries).
...
- **Editing a previous entry** preserves Lumi's reply by default. To replace it, the user re-summons Lumi in the modal and the new reply overwrites. To wipe it, the user can clear the reply in the modal (a small UX add ??out of scope for this iteration; the brief said "保�??��? lumiReply" which is what we ship).
